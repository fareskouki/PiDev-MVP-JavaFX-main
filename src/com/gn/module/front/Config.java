 ...
 */
package com.gn.module.front;

import com.gn.module.main.*;
import com.gn.App;
...
public class Config implements Initializable {
    ...
    private void invertTheme(boolean dark) {
        ...
        Front.popConfig.hide();

        Platform.runLater(() -> {
//          force pop's transition

            Front.popup.getRoot().getStylesheets().remove(Front.popup.getRoot().getStylesheets().size() - 1);
            Front.popConfig.getRoot().getStylesheets().remove(Front.popConfig.getRoot().getStylesheets().size() - 1);

            Front.popup.getRoot().getStylesheets().add(path + "pop" + theme);
            Front.popConfig.getRoot().getStylesheets().add(path + "pop" + theme);
        });
